...
csvwrite('awos_gt_300.csv',great_than_300data);
...
csvwrite('awos_ignored.csv',ignored_data);
csvwrite('awos_filtered.csv',filtered_data);
...
csvwrite('awos_interpolated.csv',interpolated_data);
...
for st=1:size(all_available_stations)
    ...
    print(strcat(mat2str(all_available_stations(st)),'_awos_org.png'),'-dpng');
    close all;
end
...
for st=1:size(all_available_stations)
    ...
    rotateXLabels(ax,45);
    title (all_available_stations(st));
    print(strcat(mat2str(all_available_stations(st)),'_awos_filt.png'),'-dpng');
    close all;
end
toc